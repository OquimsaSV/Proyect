...
<body class="fixed-nav sticky-footer" id="page-top">
  ...
  <div class="container-fluid">
    <?php
      ...
      include('navbar.php');
      ?>
    <?php include('modals/modal_usuarios.php'); ?>
    <div class="row" id="principal" class="table-responsive">
          
            
            <div class="col-md-12">
              <h2 class="text-center text-primary">Usuarios</h2>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#mana2").on('hidden.bs.modal', function () {
               $("#editRowID").val(0);
               $("#nom1").val("");
               $("#pass1").val("");
               $("#op1").val("");
            ...
            $('#cargar').load('tabla/tabla_usuarios.php');
...
//funcion para borrar un registro al cual solo se le envia el id de la fila a eliminar
        function deleteRow(rowID) {
            if (confirm('Esta seguro de borrar este usuario?')) {
                $.ajax({
                    url: 'procesar/ajax_usuarios.php',
                    method: 'POST',
                    dataType: 'text',
                    data: {
                        key: 'deleteRow',
...
//sive para enviar todos los datos del modal agregar y procesarlos, luego los reorna
        $("#agregarinfo").on("submit", function(f){
        f.preventDefault();
        $("#actualizar").fadeOut();
        var formData = new FormData(document.getElementById("agregarinfo"));
        $.ajax({
          url: "procesar/subir_usuario.php",
          type: "POST",
          dataType: "HTML",
          data: formData,
          cache: false,
          ...
          alert(echo);
          if (echo=="Usuario insertado con éxito!!") {
            $("#mana").modal('hide');
            $('#cargar').load('tabla/tabla_usuarios.php');
            getExistingData(0, 50);
          }
         ...
         $("#editarinfo").on("submit", function(e){
        e.preventDefault();
        var formData2 = new FormData(document.getElementById("editarinfo"));
        $.ajax({
          url: "procesar/actualizar_usuarios.php",
          type: "POST",
          dataType: "HTML",
          data: formData2,
          cache: false,
          ...
          processData: false,
            ...
            alert(success);
          if (success=="Usuario actualizado con éxito!!") {
              $("#mana2").modal('hide');
              $('#cargar').load('tabla/tabla_usuarios.php');
              getExistingData(0, 50);
          }
